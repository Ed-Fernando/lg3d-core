...
public class ResizeCursor extends Cursor3D {
    /**
     * Note - this method uses a naming convention to decide rotation angle
     * of the cursor shape.
     */
    public ResizeCursor(String name, float size) {
        this(name, size, 
                /*(name.startsWith("S_") || name.startsWith("N_")?((float)Math.toRadians(0)):(...*/
    ...
    public ResizeCursor(String name, float size, float dir) {
        super(name);
