...
class Quadrics extends Object {
                      ...
                      boolean yUp)
  {
	...
	if ((j == ydivisions) && (highr == 0)) {
          if (outside) {
            ...
            gbuf.texCoord2d(s, calcTextureCoordT(yUp,t));
            gbuf.vertex3d( px*r, py*r, z );
            gbuf.normal3d( 0.0, 0.0, znormal*sign );
            gbuf.texCoord2d(s+du, calcTextureCoordT(yUp,t));
  ...
  GeomBuffer disk(double r, int xdivisions, boolean outside, boolean yUp)
  {
...
//  	gbuf.texCoord2d(0.5-cosTheta*0.5, 0.5+sinTheta*0.5);
  ...
  GeomBuffer coneTop(double coneRadius, double coneHeight, int xdivisions,
		     int ydivisions, boolean outside, boolean yUp) {
    ...
    gbuf.normal3d(0.0, 0.0, znormal*sign);
    gbuf.texCoord2d(.5, calcTextureCoordT(yUp,1));
		      ...
		      boolean outside, boolean yUp)
  {
    ...
    GeomBuffer disk(double r, int xdiv, double y, boolean outside, boolean yUp) {
    ...
    GeomBuffer cylinder(double height, double radius,
			int xdiv, int ydiv, boolean outside, boolean yUp) {
...
//   	else c = 0;
	...
	for (int i = 0; i < ydiv; i++) {
		...
		gbuf.texCoord2d(s, calcTextureCoordT(yUp,t+dv));
		...
		gbuf.texCoord2d(s+du, calcTextureCoordT(yUp,t+dv));
...
// 	double dv = 1.0/(double)(ydiv+1);
    ...
    GeomBuffer coneTop(double bottom, double radius, double height,
		       int xdiv,double t, boolean outside, boolean yUp) {
...
// 	    for (int i = xdiv; i >= 0; i--) {
    ...
    private double calcTextureCoordT(boolean yUp, double t) {
        if (yUp)
            return 1.0-t;
