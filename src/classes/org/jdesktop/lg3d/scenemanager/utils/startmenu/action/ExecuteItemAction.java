 ...
 *
 *   $RCSfile: ExecuteItemAction.java,v $
 *
...
public class ExecuteItemAction extends AppLaunchAction {

    private static Logger logger = Logger.getLogger("lg.startmenu");

    private MenuItem menuItem;

    public ExecuteItemAction(MenuItem menuItem) {
        super(menuItem.getCommand(), menuItem.getClassLoader());
        this.menuItem= menuItem;
    }

    @Override
    public void performAction(LgEventSource source) {
        logger.log(Level.FINE, menuItem + " : Executing command '" + menuItem.getCommand() + "'");
        if(menuItem.getCommand() != null)
            super.performAction(source);        
        AppConnectorPrivate.getAppConnector().postEvent(new MenuItemExecutedEvent(menuItem), null);
    }
