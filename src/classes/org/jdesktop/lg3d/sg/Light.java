 ...
 * removal, and enumeration of nodes in the list of scopes. If the
 * scope list is empty, the light has universal scope and affects
 * all objects in the scene graph.
 * <p>
...
public abstract class Light extends Leaf {
	...
	((LightWrapper)wrapped).setEnable( state );
    }
	...
	return ((LightWrapper)wrapped).getEnable();
    }
	...
	((LightWrapper)wrapped).setColor( color );
    }
	...
	((LightWrapper)wrapped).getColor( color );
    }
	...
	((LightWrapper)wrapped).setScope( (GroupWrapper)scope.wrapped, index );
    }
	...
	return (Group)((LightWrapper)wrapped).getScope(index).getUserData();
    }
	...
	((LightWrapper)wrapped).insertScope( (GroupWrapper)scope.wrapped, index );
    }
	...
	((LightWrapper)wrapped).removeScope( index );
    }
	...
	Enumeration e = ((LightWrapper)wrapped).getAllScopes();
	...
	((LightWrapper)wrapped).addScope( (GroupWrapper)scope.wrapped );
    }
	...
	return ((LightWrapper)wrapped).numScopes();
    }
	...
	return ((LightWrapper)wrapped).indexOfScope( (GroupWrapper)scope.wrapped );
    }
	...
	((LightWrapper)wrapped).removeScope( (GroupWrapper)scope.wrapped );
    }
	...
	((LightWrapper)wrapped).removeAllScopes();
    }
    ...
    public void setInfluencingBounds(Bounds region) {
    	((LightWrapper)wrapped).setInfluencingBounds( region.wrapped );
    }
    ...
    public Bounds getInfluencingBounds() {
    	return (Bounds)((LightWrapper)wrapped).getInfluencingBounds().getUserData();
    }
